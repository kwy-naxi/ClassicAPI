...
{
    public class PgSqlAgent : IDbAgent
    {
        private NpgsqlConnection _connection = null;
        private NpgsqlDataAdapter _dataAdapter = null;
        private NpgsqlCommandBuilder _sqlCom = null;
        ...
        public PgSqlAgent(string connectStr)
        {
            _connection = new NpgsqlConnection(connectStr);
        }

        public PgSqlAgent(NpgsqlConnection connection)
        {
        ...
        {
            _dataAdapter = new NpgsqlDataAdapter(strQuery, _connection);
            _sqlCom = new NpgsqlCommandBuilder(_dataAdapter);
        ...
        public int ExecuteNonQuery(IDbConnection connection, IDbTransaction trx, string strQuery, IDataParameter[] paramArray, CommandType cmdType, out IDataParameterCollection col)
        {
            ...
            if (connection == null)
                cmd = new NpgsqlCommand(strQuery, _connection);
            else
                cmd = new NpgsqlCommand(strQuery, (NpgsqlConnection)connection);

            if (trx != null)
            {
                cmd.Transaction = (NpgsqlTransaction)trx;
            }
